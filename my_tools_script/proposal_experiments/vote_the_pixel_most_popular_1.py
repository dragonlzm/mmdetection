...
def get_target_single(gt_bboxes, gt_labels, points, num_classes=65):
    ...
    # mask_of_all_cates should be the number per image per pixel
...
save_path = "/home/zhuoming/label_assignment_per_cate_1/"
...
for i, image_id in enumerate(from_image_id_to_image_info):
    ...
    # mask_of_all_cates should be the number per image per pixel
    empty_pixel = (torch.sum(mask_of_all_cates, dim=0) == 0)
    # find the max idx for all pixel
    _, most_frequent = torch.max(mask_of_all_cates, dim=0)
    # assign the empty pixel
    most_frequent[empty_pixel] = 65
    
    # visualization_result = torch.cat(visualization_result, dim=0)
    visualization_result = color[most_frequent]
