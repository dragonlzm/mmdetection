...
def parse_args():
    ...
    parser.add_argument('--annofile', help='the target annotation file path')
...
if __name__ == "__main__":
    ...
    proposal_path = '/data/zhuoming/detection/coco/vitdet_proposal'
    ...
    gt_content = json.load(open(args.annofile))
    ...
    for i, image_id in enumerate(from_image_id_to_image_info):
        ...
        if not os.path.exists(prediction_path) or not os.path.exists(clip_conf_path):
            print(image_id)
            continue
        ...
        # saved file 
        mask_file_name = file_name + '_mask.pt'
        torch.save(mask_of_all_cates.cpu(), os.path.join(proposal_path, mask_file_name))
        if i % 100:
            print(i)
