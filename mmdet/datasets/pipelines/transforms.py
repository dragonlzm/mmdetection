...
from typing import Any, Callable, Dict, List, Optional, Sequence, Tuple, Union, final
import cv2
import os
import mmcv
import numpy as np
...
@PIPELINES.register_module()
class UnnormalizedImg:
    """This tranformation aims to prepare the image for the vit backbone.
    The image needed usually is the image which is unnormalized and a square
    will deliver the image in result with the key "ori_img"
    ...
    def _resize_img(self, results):
        ...
        if self.img_scale is not None:
            ...
            results['ori_img'] = copy.deepcopy(results['img'])
...
class Normalize:
    ...
    def __init__(self, mean, std, to_rgb=True, divide_255=False):
...
class GenerateCroppedPatches:
    """Generate the cropped patches for the CLIP model
        it has to be called after the resize but before the normalization
    """
...
class CopyPaste(albumentations.DualTransform):
    ...
    def bboxes_copy_paste(self, bboxes, paste_bboxes, masks, paste_masks, alpha, key):
        if key == 'paste_bboxes':
            return bboxes
        elif paste_bboxes is not None:
            masks = self.masks_copy_paste(masks, paste_masks=[], alpha=alpha)
            adjusted_bboxes = self.extract_bboxes(masks)
    ...
    def keypoints_copy_paste(self, keypoints, paste_keypoints, alpha):
    ...
    def ori_call(self, *args, force_apply: bool = False, **kwargs) -> Dict[str, Any]:
        ...
        if (random.random() < self.p) or self.always_apply or force_apply:
            ...
            kwargs['bboxes'] = merged_bboxes
            
            merged_paste_bboxes = []
            for i, (bbox, label) in enumerate(zip(kwargs['paste_bboxes'], kwargs['paste_gt_labels'])):
                bbox = bbox.tolist()
                bbox.extend([label, i])
            ...
            # 'image', 'img_shape', 'ori_shape', 'img_fields', 'bboxes', 'gt_bboxes_ignore', 'gt_labels', 'masks', 
