...
class RetinaDistillHead(AnchorHead):
    ...
    def forward_single(self, x):
        ...
        cls_feat = self.map_to_clip(reg_feat)
        # convert the dimension that fit the linear layer
        print('feature before the conversion:', cls_feat.shape)
        cls_feat = cls_feat.reshape(list(cls_feat.shape)[:-1] + [self.num_anchors, self.clip_dim])
        print('feature after the conversion:', cls_feat.shape)
        # use the linear layer for classification
        cls_score = self.retina_cls(cls_feat)
        print('cls_score after the linear classification:', cls_score.shape)
        # map the classification score back to the dimension needed by the retina        
        cls_score = cls_score.reshape(list(cls_score.shape)[:-1] + [-1, ])
        print('cls_score after the conversion:', cls_score.shape)
    ...
    def loss(self,
        ...
        # select the anchor is not meaningless to calculate the distillation loss
        label_channels = self.cls_out_channels if self.use_sigmoid_cls else 1
        cls_reg_targets = self.get_targets_idx(
            anchor_list,
            valid_flag_list,
            rand_bboxes,
            img_metas,
