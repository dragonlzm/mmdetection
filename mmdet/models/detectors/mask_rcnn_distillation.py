...
class MaskRCNNDistill(BaseDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(MaskRCNNDistill, self).__init__(init_cfg)
        #if pretrained:
    ...
    def extract_feat(self, img):
        ...
        x = self.backbone_to(img)
        if self.with_neck:
            x = self.neck(x)
        # prepare the feat from the backbone_from
        return x

    def extract_distilled_feat(self, cropped_patches_list):
        # the cropped_patches_list include the cropped patch from 
        # both gt bboxes and the random bboxes
        result_list = []
        for patches in cropped_patches_list:
            x = self.backbone_from(patches.cuda())
