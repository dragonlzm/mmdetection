...
for count_i, image_id in enumerate(from_img_id_to_bbox.keys()):
    ...
    for i in range(h_patch_num):
        ...
        for j in range(w_patch_num):
            ...
            print(type(now_patch[0][0]))
            data = im.fromarray(now_patch)
            data.save(str(image_id) + '_' + str(i) + "_" + str(j) +'.png')
...
for count_i, image_id in enumerate(from_img_id_to_bbox.keys()):
    ...
    for bbox in from_img_id_to_bbox[image_id]['bbox']:
        ...
        for now_x_idx in range(x_idx, br_x_idx+1):
            for now_y_idx in range(y_idx, br_y_idx+1):
                ...
                match_list[now_y_idx][now_x_idx].append(bbox)
    ...
    for i in range(h_patch_num):
        ...
        for j in range(w_patch_num):
            '''
            # for visulize the three bathroom patches
            if image_id == 6818 and i == 1 and j == 2:
                print(patch_count)
            if image_id == 403385 and i == 0 and j == 1:
                print(patch_count)
            if image_id == 403385 and i == 0 and j == 2:
                print(patch_count)
            patch_count += 1'''
            '''
            # for saving all the patches
            if len(match_list[i][j]) == 0:
                w_start_pos = w_pos[j]
                w_end_pos = w_pos[j+1]
                # cropping the img into the patches which size is (H/8) * (W/8)
                ...
                print(type(now_patch[0][0]))
                data = im.fromarray(now_patch)
                data.save('./patches/' + str(image_id) + '_' + str(i) + "_" + str(j) +'.png')'''

            # for saving the tooth brush and the tie
            # the tooth brush is 90
            # the tie is 32
            if final_assign_tensor[i][j] == 90 or final_assign_tensor[i][j] == 32:
                w_start_pos = w_pos[j]
                w_end_pos = w_pos[j+1]
                # cropping the img into the patches which size is (H/8) * (W/8)
                # use the numpy to crop the image
                ...
                #print(type(now_patch[0][0]))
                data = im.fromarray(now_patch)
                data.save('./toothbrush_and_tie/' + str(image_id) + '_' + str(i) + "_" + str(j) + '_' + str(final_assign_tensor[i][j]) +'.png')
