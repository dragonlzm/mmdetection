...
for count_i, image_id in enumerate(from_img_id_to_bbox.keys()):
    ...
    for i in range(h_patch_num):
        ...
        for j in range(w_patch_num):
            ...
            now_patch = np.flip(now_patch, 0)

            temp_PIL_image = Image.fromarray(np.uint8(now_patch))
            #temp_PIL_image.save('./4_by_4_original/' + str(image_id) + '_' + str(i) + "_" + str(j) + '_' + str(final_assign_tensor[i][j].item()) +'.png')
            temp_PIL_image.save('./4_by_4_original_flip/' + str(image_id) + '_' + str(i) + "_" + str(j) + '_' + str(final_assign_tensor[i][j].item()) +'.png')
            #temp_PIL_image.save('./8_by_8_original/' + str(image_id) + '_' + str(i) + "_" + str(j) + '_' + str(final_assign_tensor[i][j].item()) +'.png')
            #temp_PIL_image.save('./8_by_8_original_flip/' + str(image_id) + '_' + str(i) + "_" + str(j) + '_' + str(final_assign_tensor[i][j].item()) +'.png')
...
#np.save('newest_patches_gt_8_by_8_val.npy', all_assigned_res.reshape(-1).numpy())
...
#torch.save(all_feature_res, '000000281693_feature_4_by_4.pt')
torch.save(all_feature_res, '000000281693_feature_4_by_4_fliped.pt')
#torch.save(all_feature_res, '000000281693_feature_8_by_8.pt')
#torch.save(all_feature_res, '000000281693_feature_8_by_8_fliped.pt')
