...
# original LVIS split
# base_names = ('air_conditioner', 'airplane', 'alarm_clock', 'antenna', 'apple', 
...
for root in proposal_path_root:
    ...
    for i, image_id in enumerate(from_image_id_to_annotation):
        ...
        if novel_gt_bboxes.shape[0] != 0:
            ...
            over_05_novel += torch.sum(overlap_on_novel_gt>0.5)
            over_03_novel += torch.sum(overlap_on_novel_gt>0.3)
        ...
        if base_gt_bboxes.shape[0] != 0:
            ...
            over_05_base += torch.sum(overlap_on_base_gt>0.5)
            over_03_base += torch.sum(overlap_on_base_gt>0.3)
    ...
    print(root, 'avg iou for novel:', all_iou_for_novel / image_with_novel, 'avg iou for base:', all_iou_for_base / image_with_base,
          ...
          'over_05_novel:', over_05_novel, 'over_03_novel', over_03_novel, 'over_05_base', over_05_base, 'over_03_base', over_03_base)
