...
for count_i, image_id in enumerate(from_img_id_to_bbox.keys()):
    ...
    for bbox in from_img_id_to_bbox[image_id]['bbox']:
        ...
        #print('x, y, w, h', x, y, w, h, 'H, W', H, W, 'patch_H, patch_W', patch_H, patch_W)
        #print('br_x', br_x, 'br_y', br_y)
        #print('br_x_idx', br_x_idx, 'br_y_idx', br_y_idx)
        #print(int(br_y) / int(patch_H), math.floor(br_y / patch_H), int(br_y) / int(patch_H) == math.floor(br_y / patch_H))
...
#np.save('assigned_res.npy', all_assigned_res.reshape(-1).numpy())

np.save('assigned_res_4_by_4.npy', all_assigned_res.reshape(-1).numpy())
#torch.save(all_assigned_res, 'assigned_res.pt')
#torch.save(all_feature_res, 'feature.pt')
