...
novel_idx = [from_name_to_idx[name] for name in novel_name]
...
for proposal_file in all_proposal_files:
    ...
    novel_idx = torch.tensor([i for i, ele in enumerate(max_cate_idx) if ele in novel_idx])
    needed_objectness_score = objectness_score[novel_idx]
    clip_confidence_score = value[novel_idx]
    
    # dump the result
    final_pred_files = proposal_file[:-5] + '_final_pred.json'
    final_pred_content = json.load(open(os.path.join(file_root, final_pred_files)))
    final_pred_content['novel_idx'] = novel_idx
